...
void TrigramWidget::paintGLImpl() {
  ...
  // If ort_dist happens to be negative or 0 (ie. camera is at or past cube center),
  ...
  // the animation between ortho and perspective.  Note that point size calculation
  // in vertex shader needs to use the same factor.
  ...
  // 0.01 is a bit less than 2 / 256, making points close to each other,
  // but not touching.
  program_.setUniformValue("point_size_factor", 0.01f * std::min(width_, height_));
